...
class SegmentVisual:
    ...
    def __init__(self, color_option):
        self.palette = ade_palette(color_option)
        self.selected_labels = [3,12,42, 138]
        self.last_poly = []
        self.last_poly_image = []
    ...
    def segment_visual(self, class_queries_logits, masks_queries_logits, image):
        ...
        return img.astype(np.uint8), poly_seg, object_type

    def poly_visual(self, seg_img, image):

        poly_images = []
        epsilon_const = {0:0.01, 3: 0.01, 12: 0.007, 42: 0.01, 138:0.01}
        ...
        if len(poly_images) == 4:
            result = cv2.addWeighted(poly_images[3], 1,cv2.addWeighted(cv2.addWeighted(poly_images[0],1,poly_images[1],1,0),1,poly_images[2],1,0),1,0)
        ...
        if len(poly_images) >0:
            self.last_poly_image = result

        if len(poly_images) == 0:
            result = self.last_poly_image
